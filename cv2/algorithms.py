...
class Algorithms:
    ...
    # Function for getting angle between two lines
    ...
    def resizeRectangle(self, er: QPolygonF, pol:QPolygonF):
        ...
        #Building and Enclosing rectangle area
        pol_ar = abs(self.computeArea(pol))
        er_ar = abs(self.computeArea(er))

        # Fraction of areas of building and enclosing rectangle
        k = pol_ar/er_ar
    ...
    def minAreaEnclosingRectangle(self, pol: QPolygonF):
        ...
        for i in range(len(ch)):
            # Compute sigma
            dx = ch[(i+1)%(len(ch))].x() - ch[i].x()
            dy = ch[(i+1)%(len(ch))].y() - ch[i].y()
            sigma = atan2(dy,dx)
    ...
    def wallAverage(self, pol: QPolygonF):
        # Find pivot and first initialisation
        dx_0 = pol[1].x() - pol[0].x()
        dy_0 = pol[1].y() - pol[0].y()
        sigma0 = atan2(dy_0, dx_0)
        r_sum = 0
        l_sum = 0

        # Process all polygon vertices
        for i in range(len(pol)):
            # Calculating of sigma
            dx = pol[(i + 1) % len(pol)].x() - pol[i].x()
            dy = pol[(i + 1) % len(pol)].y() - pol[i].y()
            sigma = atan2(dy, dx)

            # State where points are identical
            if dx == 0 and dy == 0:
                continue
            else:
                # Calculating of delta sigma, k and rest
                delsig = sigma - sigma0
                k = (2*delsig) / pi
                r = delsig - (k * pi/2)
                # Updating of rest sum and lenght sum
                lenght = sqrt(dx*dx + dy*dy)
                r_sum += r * lenght
                l_sum += lenght

        # Calculating of ending sigma
        sigend = sigma0 + r_sum/l_sum
        # Rotation of building by -sigma end
        rot_pol = self.rotate(pol, -sigend)

        # Creating of MinMaxBox, their rotation by sigma end and resizing
        mmb, area = self.minMaxBox(rot_pol)
        rot_mmb = self.rotate(mmb, sigend)
        res_mmb = self.resizeRectangle(rot_mmb, pol)
